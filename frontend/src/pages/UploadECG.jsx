  ...
  const handleFileSelection = (e) => {
    if (e.target.files[0]) {
      ...
      if (!validExtensions.includes(selectedFile.type)) {
        setError("Format Error: Please attach a valid ECG image graph (PNG, JPG, or JPEG).");
        setFile(null);
        return;
      }
            ...
            <div className="lg:col-span-5 bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-gray-100 space-y-6">
              ...
              <form onSubmit={handlePipelineSubmission} className="space-y-4">
                <div className="border-2 border-dashed border-slate-100 hover:border-indigo-400 rounded-2xl p-6 transition bg-slate-50/50 hover:bg-white text-center flex flex-col items-center justify-center cursor-pointer relative group min-h-[160px]">
                  <input type="file" accept="image/png,image/jpeg" onChange={handleFileSelection} className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" disabled={isAnalyzing} />
                  ...
                  <p className="text-[10px] text-gray-400 font-semibold mt-0.5">Supports PNG, JPG, and JPEG</p>
                </div>
              ...
              {currentReport ? (
                <div className="space-y-6 animate-fadeIn">
                  ...
                  <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-4">
                    ...
                    {currentReport.probabilities && (
                      <div className="space-y-1.5">
                        <span className="text-[10px] text-gray-400 font-black uppercase tracking-wider block">Class Probability Profile</span>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {Object.entries(currentReport.probabilities).map(([label, probability]) => (
                            <div key={label} className="flex items-center justify-between gap-3 bg-slate-50 border border-slate-100 rounded-lg px-3 py-2 text-[11px]">
                              <span className="text-slate-600 truncate">{label}</span>
                              <span className="font-black text-indigo-600 shrink-0">{probability}%</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {Array.isArray(currentReport.tracePredictions) && currentReport.tracePredictions.length > 0 && (
                      <div className="text-[11px] text-slate-500">
                        Lead-row classifications: {currentReport.tracePredictions.join(" | ")}
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        <span className="text-[10px] text-gray-400 font-black uppercase tracking-wider block">AI Explanation</span>
                        <p className="text-xs text-gray-600 leading-relaxed bg-slate-50 border border-slate-100 p-3 rounded-xl">
                          {currentReport.aiExplanation || "No explanation was returned."}
                        </p>
                      </div>
                      <div className="space-y-1.5">
                        <span className="text-[10px] text-gray-400 font-black uppercase tracking-wider block">Recommended Follow-up</span>
                        <p className="text-xs text-gray-600 leading-relaxed bg-slate-50 border border-slate-100 p-3 rounded-xl">
                          {currentReport.followUp || "Follow-up guidance is unavailable."}
                        </p>
                      </div>
                    </div>
                  ...
                  {(currentReport.riskLevel?.toLowerCase() === "high" || currentReport.riskLevel?.toLowerCase() === "critical" || currentReport.prediction === "STEMI" || currentReport.prediction === "ST-Elevation Myocardial Infarction" || currentReport.prediction === "AFib") && (
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fadeIn">
              <div className="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl border border-slate-100 relative max-h-[85vh] overflow-y-auto space-y-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <span className="text-[9px] text-gray-400 font-black uppercase tracking-wider block">AI Explanation</span>
                      <p className="text-xs text-gray-600 leading-relaxed bg-slate-50/30 border border-slate-100 p-3 rounded-xl">
                        {selectedReportModal.aiExplanation || "No explanation was returned."}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] text-gray-400 font-black uppercase tracking-wider block">Recommended Follow-up</span>
                      <p className="text-xs text-gray-600 leading-relaxed bg-slate-50/30 border border-slate-100 p-3 rounded-xl">
                        {selectedReportModal.followUp || "Follow-up guidance is unavailable."}
                      </p>
                    </div>
                  </div>
